...
>
> 本表同时加入少量“补充形状”测试点（例如 ny!=64、非幂次立方体），用于验证实现不依赖特定 shape 的偶然特性。
...
| 256×96×256 | 2.97e8 | 4.91e10 | 4.22e10 | 0.86 |
| 333^3 | 2.96e8 | 5.25e10 | 4.87e10 | 0.93 |
| 384^3 | 2.68e8 | 5.26e10 | 4.95e10 | 0.94 |
| 512×64×512 | 3.00e8 | 5.10e10 | 4.66e10 | 0.91 |
| 512×128×512 | 2.93e8 | 5.26e10 | 4.84e10 | 0.92 |
...
- **补充验证（避免“只对特定形状有效”）**：额外测试了 `256×96×256`、`512×128×512` 与 `333^3`（表中已列出，均 diff=0），性能与同尺度测试一致，说明实现不依赖 `ny=64` 或幂次尺寸的特例。
- **分辨率变更（dx/dt）**：额外测试了 `256^3` 的 `dx=5,dt=0.001` 与 `dx=20,dt=0.004`（保持 `dt/dx` 不变，均 diff=0），说明在评测可能调整 `-dx/-dt` 的情况下实现仍保持正确性。
