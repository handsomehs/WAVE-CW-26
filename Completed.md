...
- **实现细节**：
  - CUDA（`src/wave_cuda.cu`）与 OpenMP（`src/wave_omp.cpp`）均提供**单核/单 offload**路径（逐点阻尼分支）以降低小规模启动开销。
  - 该阶段最初使用 `nsites` 阈值进行自动切换；后续在全量 sweep 后由第 16 节统一决定 auto 行为（保持单核为默认）。
- **正确性**：
...
- 128^3 及以上：整体性能基本保持不变（sweep 结果显示单核仍为最佳/并列最佳）。
